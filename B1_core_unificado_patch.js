...
function _B1_normTextoPlano(texto) {
  return String(texto || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function _B1_textoEquivalente(a, b) {
  return _B1_normTextoPlano(a) === _B1_normTextoPlano(b);
}

function _B1_normEstadoGeminiCore(estado) {
  const e = _B1_normTextoPlano(estado);
...
function _B1_indexarSlotsPorId(slots) {
  const mapa = {};
  (slots || []).forEach(slot => {
    if (slot && slot.slotId) mapa[slot.slotId] = slot;
  });
  return mapa;
}

function _B1_prepararResultadoRescateParaMerge(resultadoRescate, slots) {
  const slotsPorId = _B1_indexarSlotsPorId(slots);
  const rawCorrecciones = Array.isArray(resultadoRescate && resultadoRescate.correcciones)
    ? resultadoRescate.correcciones
    : [];

  const correccionesNormalizadas = [];
  const yaValidas = [];
  const noResueltasVerdad = [];
  const estadosPorSlotId = {};
  const vistos = new Set();

  rawCorrecciones.forEach(item => {
    const slotId = String(item && item.slotId || '').trim();
    if (!slotId) return;

    vistos.add(slotId);
    const slot = slotsPorId[slotId] || null;
    const original = slot ? slot.textoOriginal : (item && item.original ? item.original : '');
    const solucion = typeof item?.solucion === 'string' ? item.solucion.trim() : '';
    const estado = _B1_normEstadoGeminiCore(item && item.estado);

    if (estado === 'ya_valida' || (estado === 'aplicable' && original && solucion && _B1_textoEquivalente(original, solucion))) {
      estadosPorSlotId[slotId] = 'ya_valida';
      yaValidas.push({
        slotId,
        original,
        solucion: original || solucion,
        estado: 'ya_valida'
      });
      correccionesNormalizadas.push({
        slotId,
        original,
        solucion: original || solucion,
        estado: 'aplicable'
    ...
    if (estado === 'aplicable' && solucion) {
      estadosPorSlotId[slotId] = 'aplicable';
      correccionesNormalizadas.push({
        slotId,
        original,
        solucion,
    ...
    estadosPorSlotId[slotId] = 'no_resuelta';
    noResueltasVerdad.push(slotId);
    correccionesNormalizadas.push({
      slotId,
      original,
      solucion: '',
  ...
  Object.keys(slotsPorId).forEach(slotId => {
    if (vistos.has(slotId)) return;
    estadosPorSlotId[slotId] = 'no_resuelta';
    noResueltasVerdad.push(slotId);
    correccionesNormalizadas.push({
      slotId,
      original: slotsPorId[slotId].textoOriginal || '',
      solucion: '',
      estado: 'no_resuelta'
    });
  });
  ...
  return {
    resultadoParaMerge: Object.assign({}, resultadoRescate, { correcciones: correccionesNormalizadas }),
    yaValidas,
    noResueltasVerdad,
    estadosPorSlotId,
    slotsPorId
...
function _B1_reconciliarMergeConRescate(merge, rescateNormalizado) {
  const mergeBase = merge || {};
  const yaValidasIds = new Set((rescateNormalizado.yaValidas || []).map(x => x.slotId));

  const correccionesReales = (mergeBase.correcciones || []).filter(item => {
    if (!item || !item.slotId) return false;
    if (yaValidasIds.has(item.slotId)) return false;
    if (_B1_textoEquivalente(item.original, item.solucion)) return false;
    return true;
  });

  let roiRefsRevision = Array.isArray(mergeBase.roiRefsRevision)
    ? mergeBase.roiRefsRevision.filter(ref => rescateNormalizado.noResueltasVerdad.includes(ref.slotId))
    : [];

  rescateNormalizado.noResueltasVerdad.forEach(slotId => {
    if (!roiRefsRevision.some(x => x.slotId === slotId)) {
      roiRefsRevision.push({
        slotId,
        ref: `roi://temp/${slotId}`,
        ttlMs: 900000
      });
  ...
  return Object.assign({}, mergeBase, {
    correcciones: correccionesReales,
    yaValidas: (rescateNormalizado.yaValidas || []).slice(),
    noResueltas: (rescateNormalizado.noResueltasVerdad || []).slice(),
    roiRefsRevision
  });
}

function _B1_ajustarPasaporteSegunRescate(pasaporteBase, rescateNormalizado) {
  const unresolved = rescateNormalizado.noResueltasVerdad || [];
  const slotsPorId = rescateNormalizado.slotsPorId || {};
  ...
  const hayFamiliaSinResolver = unresolved.some(slotId => {
    const slot = slotsPorId[slotId];
    return slot && slot.categoria === 'familia';
  });

  if (hayFamiliaSinResolver) {
    return {
      estado: B1_PASSPORT.ROJO,
      explicacion: `Rescate incompleto. ${unresolved.length} alérgeno(s)/familia(s) sin resolver.`,
      accionSugeridaParaCerebro: (B1_ACCIONES_CEREBRO && (B1_ACCIONES_CEREBRO.CONTINUAR_Y_MARCAR || B1_ACCIONES_CEREBRO.REINTENTAR_UNA_VEZ)) || null
    };
  }

  return {
    estado: B1_PASSPORT.NARANJA,
    explicacion: `Rescate parcial. ${unresolved.length} formato(s) sin resolver.`,
    accionSugeridaParaCerebro: (B1_ACCIONES_CEREBRO && B1_ACCIONES_CEREBRO.CONTINUAR_Y_MARCAR) || (pasaporteBase && pasaporteBase.accionSugeridaParaCerebro) || null
  };
}
...
async function B1_analizar(input, config) {
  ...
  const sensitivityModeInterno = B1_SENSITIVITY.MEDIA;
  const agentEnabledInterno = true;
  ...
  try {
    ...
    const fiabilidad = B1_medirFiabilidad(ocrNormalizado, sensitivityModeInterno);
    ...
    const loteRescate = B1_prepararLoteRescate(
      fiabilidad.palabrasDudosas,
      fiabilidad.candidatosRiesgo || [],
      canvasListo,
      sensitivityModeInterno
    );
    ...
    tiempos.cliente.t_total_candidatos_riesgo = (fiabilidad.candidatosRiesgo || []).length;
    tiempos.cliente.t_total_entradas_selector =
      tiempos.cliente.t_total_dudas_detectadas +
      tiempos.cliente.t_total_candidatos_riesgo;
    ...
    const rescateNormalizado = _B1_prepararResultadoRescateParaMerge(resultadoRescate, loteRescate.slots);

    const tMerge = Date.now();
    const mergeRaw = B1_ejecutarMerge(textoBase, loteRescate.slots, rescateNormalizado.resultadoParaMerge);
    const merge = _B1_reconciliarMergeConRescate(mergeRaw, rescateNormalizado);
    tiempos.cliente.t_merge_ms = Date.now() - tMerge;

    B1_diagMerge(diag, merge, Date.now() - startTime, tiempos.cliente.t_merge_ms);

    let pasaporte = B1_emitirPasaporte(merge, fiabilidad, agentEnabledInterno, cronometro, null);
    pasaporte = _B1_ajustarPasaporteSegunRescate(pasaporte, rescateNormalizado);
