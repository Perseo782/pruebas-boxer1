...
async function B1_analizar(input, config) {
  ...
  const tiempos = {
    cliente: {}, upstream: { vision: null, gemini: null },
  ...
  try {
    ...
    if (tiempos.upstream.vision && typeof tiempos.upstream.vision.t_total_trastienda_ms === 'number' && typeof tiempos.cliente.t_fetch_vision_total_ms === 'number') {
      tiempos.estimaciones.t_vision_externo_no_desglosado_ms = Math.max(0, tiempos.cliente.t_fetch_vision_total_ms - tiempos.upstream.vision.t_total_trastienda_ms);
    }
    ...
    if (!datos.agentEnabled) {
      ...
      const metricas = B1_crearMetricas({ pageConfidence:fiabilidad.pageConfidence, criticalZoneScore:fiabilidad.criticalZoneScore, elapsedMs, abortReason:null, tiempos });
      return B1_crearRespuestaOk({ textoBaseVision:textoBase, textoAuditado:textoBase, estadoPasaporte:B1_PASSPORT.NARANJA, metricas, traceId, elapsedMs, accionSugeridaParaCerebro:B1_ACCIONES_CEREBRO.CONTINUAR_Y_MARCAR, warning:'Agente desactivado. Resultado usable con OCR base.', diagnostico:B1_exportarDiagnostico(diag) });
    }

    if (cronometro.expired()) return _respuestaTimeout('antes_slots', traceId, startTime, textoBase, fiabilidad, diag, tiempos);
    ...
    if (loteRescate.totalSlots === 0) {
      const elapsedMs = Date.now() - startTime;
      tiempos.total.t_total_boxer1_ms = elapsedMs;
      const metricas = B1_crearMetricas({ pageConfidence:fiabilidad.pageConfidence, criticalZoneScore:fiabilidad.criticalZoneScore, elapsedMs, tiempos });
      ...
      return B1_crearRespuestaOk({ textoBaseVision:textoBase, textoAuditado:textoBase, estadoPasaporte:B1_PASSPORT.VERDE, metricas, traceId, elapsedMs, diagnostico:B1_exportarDiagnostico(diag) });
    }
    ...
    if (resultadoRescate.upstreamError) B1_diagEncadenado(diag, resultadoRescate.upstreamError.upstreamCode || 'UPSTREAM_UNKNOWN', resultadoRescate.upstreamError.upstreamModule || 'TRASTIENDA', Date.now() - startTime, 0);
    ...
    const elapsedMs = Date.now() - startTime;
    tiempos.total.t_total_boxer1_ms = elapsedMs;
    if (resultadoRescate.upstreamError && (pasaporte.estado === B1_PASSPORT.VERDE || pasaporte.estado === B1_PASSPORT.NARANJA)) B1_diagRecuperacion(diag, B1_PASSPORT.ROJO, pasaporte.estado, elapsedMs, 'Hubo fallo upstream en rescate pero Boxer 1 devolvió salida utilizable.', 0);
    B1_diagPasaporte(diag, pasaporte.estado, pasaporte.explicacion, elapsedMs, 0);

    const metricas = B1_crearMetricas({ pageConfidence:fiabilidad.pageConfidence, criticalZoneScore:fiabilidad.criticalZoneScore, slotsEnviados:resultadoRescate.slotsEnviados, slotsDevueltos:resultadoRescate.slotsDevueltos, mergeStatus:merge.mergeStatus, rescatesIntentados:resultadoRescate.slotsEnviados, rescatesAplicados:merge.correcciones.length, elapsedMs, abortReason:null, tiempos });
...
function _respuestaTimeout(fase, traceId, startTime, textoBase, fiabilidad, diag, tiempos) {
  const elapsedMs = Date.now() - startTime;
  if (tiempos && tiempos.total) tiempos.total.t_total_boxer1_ms = elapsedMs;
  B1_diagTimeout(diag, fase, elapsedMs, 0);
  const metricas = fiabilidad ? B1_crearMetricas({ pageConfidence:fiabilidad.pageConfidence, criticalZoneScore:fiabilidad.criticalZoneScore, elapsedMs, abortReason:'Presupuesto de tiempo agotado.', tiempos }) : B1_crearMetricas({ elapsedMs, abortReason:'Presupuesto de tiempo agotado.', tiempos });
  if (textoBase) {
