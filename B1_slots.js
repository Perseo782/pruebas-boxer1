...
// ─── EXCLUSIÓN CERRADA PARA RUTA DE NOMBRE ─────────────────
// Tokens (compactados, sin acentos, minúsculas) que NUNCA pueden
// entrar como candidato_agente_nombre aunque pasen todas las puertas.
// Orden de trabajo: Recalibración Ruta Nombre (abril 2026).
const B1_EXCLUSION_NOMBRE = new Set([
  // Conservación y preparación
  'conservar', 'refrigerar', 'congelar', 'descongelar', 'consumir',
  'preferentemente', 'calentar', 'precalentar', 'freir', 'hornear',
  'hervir', 'cocer', 'reposar', 'minutos', 'segundos', 'temperatura',
  'mantener', 'guardar', 'almacenar', 'tapar', 'abrir',
  // Instrucciones y verbos de slogan
  'descubre', 'disfruta', 'prueba', 'saborea', 'sirve', 'agita',
  'mezcla', 'anade', 'vierte', 'calienta', 'cocina', 'prepara',
  // Legal y empresarial
  'fabricado', 'elaborado', 'distribuido', 'envasado', 'importado',
  'producido', 'empresa', 'industria', 'direccion', 'telefono',
  // Registro y trazabilidad
  'rgseaa', 'rsseaa', 'lote', 'referencia',
  // Nutricional residual (por si escapa de detección de zona)
  'calorias', 'kilocalorias', 'proteinas', 'hidratos', 'grasas',
  'saturadas', 'fibra', 'sodio',
  // Web y contacto
  'www', 'http', 'https', 'email', 'correo'
]);
...
function B1_mapearCategoria(decision) {
  ...
  if (d.includes('familia')) return 'familia';
  if (d.includes('peso_formato')) return 'peso_formato';
  if (d.includes('nombre')) return 'nombre';
  if (d.includes('contexto_critico')) return 'contexto_critico';
  return 'contexto_critico';
}

function _B1_tienePatronWebEmailNombre(original) {
  const txt = String(original || '').trim().toLowerCase();
  if (!txt) return false;
  return txt.includes('@') || txt.includes('www.') || txt.includes('http://') || txt.includes('https://') || txt.includes('.com') || txt.includes('.es');
}

function _B1_bloquePareceNarrativoNoNombre(contexto) {
  if (!contexto) return false;
  if (contexto.zona === 'nutricional' || contexto.zona === 'peso_formato' || contexto.zona === 'irrelevante') return true;

  const bloque = String(contexto.bloqueNorm || '');
  if (!bloque) return false;

  const marcasNarrativas = [
    'ingred', 'ingredient', 'nutrition', 'nutric', 'valor energet', 'kcal',
    'grasas', 'hidratos', 'proteinas', 'proteinas', 'sal',
    'conservar', 'consumir preferentemente', 'antes del fin',
    'freidora', 'horno', 'precalentar', 'calentar', 'descongelar',
    'fabricado por', 'elaborado por', 'producido por', 'lote', 'net weight', 'poids net'
  ];
  return marcasNarrativas.some(k => bloque.includes(_B1_normalizarComparacion(k)));
}

function _B1_calcularZonaAltaNombre(p, contexto) {
  const blockIndex = Number(p && p.blockIndex);
  const wordIndex = Number(p && p.wordIndex);
  const blockWords = Number(contexto && contexto.blockWordCount) || 0;
  const narrativo = _B1_bloquePareceNarrativoNoNombre(contexto);

  if (!Number.isFinite(blockIndex)) {
    return { score: 0, tipo: null, narrativo };
  }

  if (blockIndex <= 1 && blockWords > 0 && blockWords <= 8) {
    return { score: 120, tipo: 'plena', narrativo };
  }

  if (blockIndex <= 3 && blockWords > 0 && blockWords <= 5 && !narrativo) {
    return { score: 80, tipo: 'extendida', narrativo };
  }

  if (blockIndex <= 4 && Number.isFinite(wordIndex) && wordIndex <= 1 && blockWords > 0 && blockWords <= 4 && !narrativo) {
    return { score: 50, tipo: 'fallback', narrativo };
  }
...
// RECALIBRADO — Orden de trabajo: Recalibración Ruta Nombre (abril 2026)
//
// Cambios respecto a la versión anterior:
//   - Se elimina ratioMayusculas >= 0.45.
//   - Se elimina contexto.zona === 'neutral'.
//   - Se añaden blockIndex y wordIndex como criterios de posición.
//   - Zona alta robusta (misma lógica que _B1_puntuarNombreProducto).
//   - Exclusión cerrada B1_EXCLUSION_NOMBRE.
//   - Motivos de rechazo específicos por condición fallida.
//
// Filosofía: esta función CONFIRMA lo que la puntuación ya evaluó.
// Usa las mismas señales base. No contradice.
//
function _B1_aprobarNombreEnValidacionFinal(decision, contexto) {
  ...
  if (!token) {
    return { aprobado: false, motivo: 'nombre_rechazado_sin_token', detalle: { token } };
  }

  if (token.length < 4) {
    return { aprobado: false, motivo: 'nombre_rechazado_longitud', detalle: { token, longitud: token.length } };
  }

  if (/\d/.test(token) || _B1_esDatoNoNombreNiFamilia(original, token)) {
    return { aprobado: false, motivo: 'nombre_rechazado_dato', detalle: { token, original } };
  }

  if (B1_STOPWORDS_NOMBRE.has(token) || B1_EXCLUSION_NOMBRE.has(token) || _B1_tienePatronWebEmailNombre(original)) {
    return { aprobado: false, motivo: 'nombre_rechazado_exclusion', detalle: { token, original } };
  }

  if (contexto.zona === 'nutricional' || contexto.zona === 'peso_formato' || contexto.zona === 'irrelevante') {
    return { aprobado: false, motivo: 'nombre_rechazado_zona', detalle: { token, contextoZona: contexto.zona } };
  }

  if (contexto.blockWordCount > 8) {
    return { aprobado: false, motivo: 'nombre_rechazado_bloque_largo', detalle: { token, blockWordCount: contexto.blockWordCount } };
  }

  const senalZona = _B1_calcularZonaAltaNombre(decision, contexto);
  const wordIndex = Number(decision && decision.wordIndex);
  const primerasPosiciones = Number.isFinite(wordIndex) && wordIndex <= 1;

  if (!senalZona.score && !primerasPosiciones) {
    return {
      aprobado: false,
      motivo: 'nombre_rechazado_sin_senal_estructura',
      detalle: { token, blockIndex: decision.blockIndex, wordIndex: decision.wordIndex, contextoZona: contexto.zona }
    };
  }

  if (token.length === 4 && !(senalZona.score && primerasPosiciones)) {
    return {
      aprobado: false,
      motivo: 'nombre_rechazado_token4_sin_contexto_fuerte',
      detalle: { token, blockIndex: decision.blockIndex, wordIndex: decision.wordIndex, zonaAlta: senalZona.tipo || null }
    };
  }

  return {
    aprobado: true,
    motivo: 'nombre_validado',
    detalle: {
      token,
      blockWordCount: contexto.blockWordCount,
      contextoZona: contexto.zona,
      zonaAlta: senalZona.tipo || null,
...
//
// Cambios respecto a la versión anterior:
//   - zona !== 'neutral' ya no es puerta dura. Bloquean: nutricional, peso_formato, irrelevante.
//   - ratioMayusculas eliminado como requisito. Pasa a bonificación.
//   - blockWordCount sube de 6 a 8.
//   - Zona alta robusta: plena (0-1), extendida (2-3), fallback (2-4).
//   - Exclusión cerrada B1_EXCLUSION_NOMBRE.
//   - Umbral de entrada: score >= 160.
//   - Anti-slogan: tokens de exclusión + verificación de bloque compatible.
//
function _B1_puntuarNombreProducto(p, tokenOriginal, tokenCompacto, contexto) {
  ...
  if (_B1_esDatoNoNombreNiFamilia(tokenOriginal, tokenCompacto)) return { score: 0, motivo: null, evidencia: null };

  if (B1_STOPWORDS_NOMBRE.has(tokenCompacto) || B1_EXCLUSION_NOMBRE.has(tokenCompacto)) {
    return { score: 0, motivo: null, evidencia: null };
  }

  if (_B1_tienePatronWebEmailNombre(tokenOriginal)) {
    return { score: 0, motivo: null, evidencia: null };
  }
  ...
  const senalZona = _B1_calcularZonaAltaNombre(p, contexto);
  const wordIndex = Number(p && p.wordIndex);
  const primerasPosiciones = Number.isFinite(wordIndex) && wordIndex <= 1;
  ...
  let score = 0;
  score += senalZona.score;

  if (primerasPosiciones) score += 60;
  else if (Number.isFinite(wordIndex) && wordIndex <= 2) score += 20;

  if (contexto.blockWordCount >= 1 && contexto.blockWordCount <= 5) score += 50;
  else if (contexto.blockWordCount <= 8) score += 25;

  if (tokenCompacto.length >= 5) score += 35;
  else if (tokenCompacto.length === 4) score += 15;

  const texto = String(tokenOriginal || '').trim();
  const primeraMayus = /^[A-ZÁÉÍÓÚÜÑÀÈÌÒÙÇ]/u.test(texto);
  const mayusTotales = /^[A-ZÁÉÍÓÚÜÑÀÈÌÒÙÇ\s.-]+$/u.test(texto) && /[A-ZÁÉÍÓÚÜÑÀÈÌÒÙÇ]/u.test(texto);
  if (primeraMayus || mayusTotales) score += 25;

  if (!_B1_bloquePareceNarrativoNoNombre(contexto)) score += 40;

  const bloqueNorm = String(contexto.bloqueNorm || '');
  const cabeceraComercial = (
    !bloqueNorm.includes('ingred') &&
    !bloqueNorm.includes('nutrition') &&
    !bloqueNorm.includes('nutric') &&
    !bloqueNorm.includes('valor energet') &&
    !bloqueNorm.includes('kcal') &&
    !bloqueNorm.includes('conservar') &&
    !bloqueNorm.includes('lote')
  );
  if (cabeceraComercial) score += 20;

  if (score < 160) {
    return { score: 0, motivo: null, evidencia: `score=${score}` };
  }

  return {
    score,
    motivo: 'nombre_por_estructura',
    evidencia: `zonaAlta=${senalZona.tipo || 'no'};blockWords=${contexto.blockWordCount};wordIndex=${Number.isFinite(wordIndex) ? wordIndex : 'na'};score=${score}`
  };
}
...
function _B1_ejecutarSelectorOCR(palabrasDudosas, sensitivityMode, maxSlots) {
  ...
  // ── PRIORIDAD DURA POR CATEGORÍA ──────────────────────────
  // Orden de trabajo: Recalibración Ruta Nombre (abril 2026)
  // familia(4) > peso_formato(3) > nombre(2) > contexto_critico(1)
  // Dentro de la misma categoría: score descendente.
  // Desempate final: posición OCR ascendente.
  var _pesoCategoria = {
    'candidato_agente_familia': 4,
    'candidato_agente_peso_formato': 3,
    'candidato_agente_nombre': 2,
    'candidato_agente_contexto_critico': 1
  };

  candidatas.sort(function(a, b) {
    var catA = _pesoCategoria[a.decision.decision] || 0;
    var catB = _pesoCategoria[b.decision.decision] || 0;
...
// ATENCIÓN: acepta también candidatosRiesgo como segunda lista.
// Si se llama con la firma antigua (sin candidatosRiesgo), funciona
// igual que antes — los candidatos de riesgo simplemente no entran.
// Para el nuevo flujo completo, pasar ambas listas.
function B1_filtrarDudasRescatables(palabrasDudosas, candidatosRiesgo, sensitivityMode) {
  // Compatibilidad con llamadas antiguas: si candidatosRiesgo es string,
  // es el sensitivityMode del contrato viejo — reordenar parámetros.
  ...
  // Unificar y deduplicar igual que en B1_prepararLoteRescate
  const mapaOrigen = {};
...
function B1_prepararLoteRescate(palabrasDudosas, candidatosRiesgo, canvas, sensitivityMode) {
  ...
  // Marcar origen de cada palabra antes de unificar.
  // Si una palabra está en ambas listas, el origen es 'ambas'.
  const mapaOrigen = {};
  ...
  // Mapa posición → categoria para incrustarla en cada slot
  // selectorOCR.enviadoAgente tiene pageIndex, blockIndex, wordIndex y decision
  const mapaCategoria = {};
  if (selector.selectorOCR && Array.isArray(selector.selectorOCR.enviadoAgente)) {
    selector.selectorOCR.enviadoAgente.forEach(function(d) {
      var clave = d.pageIndex + ':' + d.blockIndex + ':' + d.wordIndex;
  ...
  const slotsPreparados = slots.map(slot => {
    ...
    return {
      ...
      categoria:       mapaCategoria[clave] || 'contexto_critico'
    };
  });
