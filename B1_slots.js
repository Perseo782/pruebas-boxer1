...
const B1_FAMILIAS_PRIORITARIAS = [
  ...
  },
  {
    id: 'huevo',
    stems: [
      'huevo', 'huevos', 'egg', 'eggs', 'oeuf', 'oeufs', "d'oeuf", 'ovo', 'ovos',
...
function _B1_compactarExactoFamilia(texto) {
  return String(texto || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function _B1_esVarianteFamiliaExacta(textoOriginal) {
  const tokenExacto = _B1_compactarExactoFamilia(textoOriginal);
  if (!tokenExacto) return { exacta: false, familia: null, stem: null };

  for (const familia of B1_FAMILIAS_PRIORITARIAS) {
    for (const stem of familia.stems) {
      const stemExacto = _B1_compactarExactoFamilia(stem);
      if (stemExacto && tokenExacto === stemExacto) {
        return {
          exacta: true,
          familia: familia.id,
          stem
  ...
  return { exacta: false, familia: null, stem: null };
}
...
function _B1_evaluarCandidatoFamiliaOCR(p) {
  ...
  const exactaValida = _B1_esVarianteFamiliaExacta(textoOriginal);
  if (exactaValida.exacta) return vacio;
