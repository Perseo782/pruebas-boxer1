(function initRevisionUi(globalScope) {
  ...
  function applyViewerTransform(state) {
    var v = state.viewer;
    var t = "translate(" + v.offsetX.toFixed(1) + "px, " + v.offsetY.toFixed(1) + "px) scale(" + v.zoom.toFixed(2) + ")";
    state.el.viewerImage.style.transform = t;
    state.el.viewerMeta.textContent = "Zoom x" + v.zoom.toFixed(2);
  }
  ...
  function openViewer(state, src, refText) {
    ...
    state.el.viewerTitle.textContent = "Foto ampliada";
    state.el.viewerMeta.textContent = refText || "";
