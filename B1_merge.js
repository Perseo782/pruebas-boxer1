...
function B1_ejecutarMerge(textoBase, slotsEnviados, resultadoRescate) {
  ...
  slotsEnviados.forEach(slot => {
    ...
    if (correccion.estado === B1_SLOT_STATUS.APLICABLE || correccion.estado === B1_SLOT_STATUS.APLICADA) {
      // Aplicar corrección: reemplazar texto original por solución
      if (correccion.solucion && correccion.solucion.trim()) {
        textoAuditado = _reemplazarPorSlot(textoAuditado, slot.textoOriginal, correccion.solucion);
        correccionesAplicadas.push({
          slotId:   slot.slotId,
          original: slot.textoOriginal,
          solucion: correccion.solucion,
